...
class PCRangeCacheTest : public CommonRuntimeTest {
 protected:
  void SetUp() override {
    ...
    // Set up memory for fake method headers
    header_memory_ = std::make_unique<ArenaAllocator>(&pool_);
  }
  ...
  std::pair<OatQuickMethodHeader*, const void*> CreateFakeMethodHeader(size_t code_size) {
    ...
    uint8_t* aligned_memory = reinterpret_cast<uint8_t*>(
        RoundUp(reinterpret_cast<uintptr_t>(memory), alignment));
...
TEST_F(PCRangeCacheTest, BasicLookup) {
  ...
  auto [header, code_ptr] = CreateFakeMethodHeader(64);
...
TEST_F(PCRangeCacheTest, InvalidateMethod) {
  auto [header, code_ptr] = CreateFakeMethodHeader(128);
...
TEST_F(PCRangeCacheTest, MultipleEntries) {
  ...
  for (size_t i = 0; i < kNumMethods; ++i) {
    auto [header, code_ptr] = CreateFakeMethodHeader(64 + i * 16);
    ArtMethod* method = CreateFakeMethod(i + 10);

    headers.push_back({header, code_ptr});
    methods.push_back(method);
    code_starts.push_back(reinterpret_cast<uintptr_t>(code_ptr));
...
TEST_F(PCRangeCacheTest, ClearCache) {
  ...
  for (size_t i = 0; i < 5; ++i) {
    ...
    cache_->FastLookup(reinterpret_cast<uintptr_t>(code_ptr));
  }
  ...
  for (size_t i = 0; i < 5; ++i) {
    auto [header, code_ptr] = CreateFakeMethodHeader(64);
    OatQuickMethodHeader* result = cache_->FastLookup(reinterpret_cast<uintptr_t>(code_ptr));
    EXPECT_EQ(result, nullptr);
  }
}
...
TEST_F(PCRangeCacheTest, ARM64AlignmentTest) {
  ...
  ArtMethod* method = CreateFakeMethod(400);
  cache_->AddEntry(code_ptr, method);

  // Test lookups at 4-byte boundaries
  for (size_t offset = 0; offset < 128; offset += 4) {
    OatQuickMethodHeader* result = cache_->FastLookup(code_start + offset);
    EXPECT_EQ(result, header) << "Failed at 4-byte aligned offset " << offset;
  }
}
#elif defined(__riscv)
TEST_F(PCRangeCacheTest, RISCVAlignmentTest) {
  // RISC-V instructions are typically 4-byte aligned
  auto [header, code_ptr] = CreateFakeMethodHeader(256);
  uintptr_t code_start = reinterpret_cast<uintptr_t>(code_ptr);

  ArtMethod* method = CreateFakeMethod(500);
  cache_->AddEntry(code_ptr, method);

  // Test cache behavior with RISC-V instruction boundaries
  for (size_t offset = 0; offset < 256; offset += 4) {
    OatQuickMethodHeader* result = cache_->FastLookup(code_start + offset);
    EXPECT_EQ(result, header) << "RISC-V cache failed at offset " << offset;
  }
}
#endif
