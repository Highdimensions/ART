...
inline void Thread::CheckSuspend(bool implicit) {
  ...
  while (true) {
    // Memory_order_relaxed is OK, since RunCheckpointFunction shares a lock with the requestor,
    // and FullSuspendCheck() re-checks later. We currently need memory_order_acquire for the
