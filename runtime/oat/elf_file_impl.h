...
class ElfFileImpl : public ElfFile {
 public:
  ...
  uint64_t FindSymbolAddress(Elf_Word section_type,
                             const std::string& symbol_name,
                             bool build_map) override;
  ...
  bool GetSectionOffsetAndSize(const char* section_name,
                               uint64_t* offset,
                               uint64_t* size) const override;

  bool HasSection(const std::string& name) const override;
