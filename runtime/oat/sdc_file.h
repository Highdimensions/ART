...
//
// 1. There may be one SDC file accompanying each SDM file. An SDC file without a corresponding SDM
//    file is garbage.
// 2. They are always local on device.
// 3. They are only read and written by the ART module.
// 4. A later version of the ART module doesn't necessarily need to understand the contents, but
//    must be able to read them without unexpected behaviors such as crashing.
//
...
class SdcReader {
 public:
  ...
  // The value of `Runtime::GetApexVersions` at the time where the SDM file is first seen on device.
  // This is for detecting samegrade placebos.
