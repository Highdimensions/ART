...
TEST_P(OatFileAssistantTest, SdmUpToDate) {
  ...
  ASSERT_NO_FATAL_FAILURE(
      CreateSecureDexMetadataCompanion(runtime_->GetApexVersions(), sdc_location));
...
// Case: We have SDM, DM, and SDC files for an uncompressed DEX file. Meanwhile, we have an VDEX
// file that is also up to date.
...
// Case: We have SDM, DM, and SDC files for an compressed DEX file.
...
TEST_P(OatFileAssistantTest, SdmApexVersionMismatch) {
  ...
  ASSERT_NO_FATAL_FAILURE(GenerateSdmDmForTest(dex_location,
                                               ...
                                               /*include_app_image=*/true,
                                               /*compilation_reason=*/"cloud"));
  ASSERT_NO_FATAL_FAILURE(CreateSecureDexMetadataCompanion("wrong-apex-version", sdc_location));
