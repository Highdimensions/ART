...
std::unique_ptr<SdcReader> SdcReader::Load(const std::string filename, std::string* error_msg) {
  ...
  if ((it = map.find("sdm-timestamp")) == map.end()) {
    *error_msg = ART_FORMAT("Missing key 'sdm-timestamp' from sdc file '{}'", filename);
    return nullptr;
  }
...
bool SdcWriter::Save(std::string* error_msg) {
  ...
  if (!WriteStringToFd(ART_FORMAT("sdm-timestamp={}\n", sdm_timestamp_), file_.Fd()) ||
      !WriteStringToFd(ART_FORMAT("apex-versions={}\n", apex_versions_), file_.Fd())) {
