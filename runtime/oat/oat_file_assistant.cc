  ...
  if (!UseFdToReadFiles()) {
    ...
    if (DexLocationToOatFilename(dex_location_,
      ...
      vdex_for_oat_.emplace(this,
                            ...
                            /*is_oat_location=*/true,
                            /*use_fd=*/false);
      dm_.emplace(this, GetDmFilename(dex_location_));
...
int OatFileAssistant::GetDexOptNeeded(CompilerFilter::Filter target_compiler_filter,
  ...
  if (dexopt_needed != kNoDexOptNeeded && dm_.has_value() && &info == &dm_.value()) {
...
OatFileAssistant::OatStatus OatFileAssistant::OdexFileStatus() {
  return odex_.has_value() ? odex_->Status() : kOatCannotOpen;
}

OatFileAssistant::OatStatus OatFileAssistant::OatFileStatus() {
  return oat_.has_value() ? oat_->Status() : kOatCannotOpen;
}
...
OatFileAssistant::OatFileInfo& OatFileAssistant::GetBestInfo() {
  ...
  if (oat_.has_value() && oat_->FileExists()) {
    log_status("odex in dalvik-cache", &oat_.value());
    if (oat_->IsUseable()) {
      return *oat_;
    }
  }
  ...
  if (odex_.has_value() && odex_->FileExists()) {
    log_status("odex next to the dex file", &odex_.value());
    if (odex_->IsUseable()) {
      return *odex_;
    }
  }
  ...
  if (vdex_for_odex_.has_value() && vdex_for_odex_->FileExists()) {
    log_status("vdex next to the dex file", &vdex_for_odex_.value());
  ...
  if (dm_.has_value() && dm_->FileExists()) {
    log_status("dm", &dm_.value());
    if (dm_->IsUseable()) {
      return *dm_;
    }
  }
...
const std::string* OatFileAssistant::OatFileInfo::Filename() { return &filename_; }
...
std::unique_ptr<OatFile> OatFileAssistant::OatFileInfoBackedByDm::LoadFile(std::string* error_msg) {
  // Check to see if there is a vdex file we can make use of.
...
OatFileAssistant::Location OatFileAssistant::GetLocation(OatFileInfo& info) {
  if (info.IsUseable()) {
    if (dm_.has_value() && &info == &dm_.value()) {
      return kLocationDm;
