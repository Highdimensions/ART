...
OatFileAssistant::OatFileAssistant(const char* dex_location,
  ...
  if (!oat_file_name.empty() && !UseFdToReadFiles()) {
    ...
    info_list_.emplace_back("odex in dalvik-cache",
                            std::make_unique<OatFileInfoBackedByOat>(this,
                                                                     oat_file_name,
                                                                     /*is_oat_location=*/true,
                                                                     /*use_fd=*/false));

    info_list_.emplace_back(
        "sdm with sdc in dalvik-cache",
  ...
  if (!odex_file_name.empty()) {
    ...
    info_list_.emplace_back("odex next to the dex file",
                            std::make_unique<OatFileInfoBackedByOat>(this,
                                                                     odex_file_name,
    ...
    info_list_.emplace_back(
        "sdm with sdc next to the dex file",
        std::make_unique<OatFileInfoBackedBySdm>(this,
                                                 GetSdmFilename(dex_location_, isa),
                                                 /*is_oat_location=*/false,
                                                 GetDmFilename(dex_location_),
  ...
  if (!oat_file_name.empty() && !UseFdToReadFiles()) {
    ...
    info_list_.emplace_back(
        "vdex in dalvik-cache",
        std::make_unique<OatFileInfoBackedByVdex>(this,
  ...
  if (!UseFdToReadFiles()) {
    ...
    info_list_.emplace_back(
        "dm", std::make_unique<OatFileInfoBackedByDm>(this, GetDmFilename(dex_location_)));
  }
}
...
int OatFileAssistant::GetDexOptNeeded(CompilerFilter::Filter target_compiler_filter,
  ...
  if (dexopt_needed != kNoDexOptNeeded && info.GetType() == OatFileInfoBackedByDm::kType) {
...
OatFileAssistant::OatStatus OatFileAssistant::OdexFileStatus() {
  for (const auto& [unused, info] : info_list_) {
    if (info->GetType() == OatFileInfoBackedByOat::kType && !info->IsOatLocation()) {
      return info->Status();
    }
  }
...
OatFileAssistant::OatFileInfo& OatFileAssistant::GetBestInfo() {
  ...
  for (auto& [location, info] : info_list_) {
  ...
  for (auto& [unused, info] : info_list_) {
    if (info->GetType() == OatFileInfoBackedByOat::kType && info->Status() != kOatCannotOpen) {
      return *info;
    }
  }
  return empty_info_;
...
OatFileAssistant::Location OatFileAssistant::GetLocation(OatFileInfo& info) {
  if (info.IsUseable()) {
    if (info.GetType() == OatFileInfoBackedByDm::kType) {
      return kLocationDm;
