...
OatFileAssistant::OatFileAssistant(const char* dex_location,
  ...
  if (DexLocationToOdexFilename(dex_location_, isa_, &odex_file_name, &error_msg)) {
    odex_ = std::make_unique<OatFileInfoBackedByOat>(this,
  ...
  if (!UseFdToReadFiles()) {
    ...
    if (DexLocationToOatFilename(dex_location_,
                                 ...
                                 &oat_file_name,
                                 &error_msg)) {
      oat_ = std::make_unique<OatFileInfoBackedByOat>(this,
...
OatFileAssistant::OatFileInfo& OatFileAssistant::GetBestInfo() {
  ...
  // If the oat location is useable, take it. This must be an app on a readonly filesystem (a
  // system app, an incremental app, etc.). This must be prioritized over the odex location, because
  // the odex location probably has the dexpreopt artifacts.
