...
OatFileAssistant::OatFileInfo& OatFileAssistant::GetBestInfo() {
  ...
  if (oat_->FileExists()) {
    log_status("odex in dalvik-cache", oat_.get());
    if (oat_->IsUseable()) {
      return *oat_;
    }
  }
  ...
  if (odex_->FileExists()) {
    log_status("odex next to the dex file", odex_.get());
    if (odex_->IsUseable()) {
      return *odex_;
    }
  }
  ...
  if (vdex_for_odex_->FileExists()) {
    log_status("vdex next to the dex file", vdex_for_odex_.get());
