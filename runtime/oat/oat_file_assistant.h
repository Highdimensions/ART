 ...
 private:
  class OatFileInfo {
   public:
    ...
    OatFileInfo() : oat_file_assistant_(nullptr), filename_(""), is_oat_location_(false) {}
   ...
   protected:
    ...
    // Loads the file.
    virtual std::unique_ptr<OatFile> LoadFile(std::string* error_msg [[maybe_unused]]) {
      return nullptr;
    }
  ...
  class OatFileInfoBackedBySdm : public OatFileInfo {
   public:
    OatFileInfoBackedBySdm(OatFileAssistant* oat_file_assistant,
        ...
        : OatFileInfo(oat_file_assistant, is_oat_location, sdm_filename),
  ...
  std::unique_ptr<OatFileInfo> odex_ = std::make_unique<OatFileInfo>();
  // The AOT-compiled file of an app when the APK of the app is on a read-only partition
  // (for example /system).
  std::unique_ptr<OatFileInfo> oat_ = std::make_unique<OatFileInfo>();

  // The SDM file containing the AOT-compiled file of an app when the APK of the app is in /data.
  std::unique_ptr<OatFileInfo> sdm_for_odex_ = std::make_unique<OatFileInfo>();
  ...
  std::unique_ptr<OatFileInfo> sdm_for_oat_ = std::make_unique<OatFileInfo>();

  // The vdex-only file next to `odex_` when `odex_' cannot be used (for example
  // it is out of date).
  std::unique_ptr<OatFileInfo> vdex_for_odex_ = std::make_unique<OatFileInfo>();
  // The vdex-only file next to 'oat_` when `oat_' cannot be used (for example
  // it is out of date).
  std::unique_ptr<OatFileInfo> vdex_for_oat_ = std::make_unique<OatFileInfo>();

  // The vdex-only file next to the apk.
  std::unique_ptr<OatFileInfo> dm_for_odex_ = std::make_unique<OatFileInfo>();
  std::unique_ptr<OatFileInfo> dm_for_oat_ = std::make_unique<OatFileInfo>();
