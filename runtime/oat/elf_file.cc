...
ElfFileImpl<ElfTypes>* ElfFileImpl<ElfTypes>::Open(File* file,
  ...
  std::unique_ptr<ElfFileImpl<ElfTypes>> elf_file(
      new ElfFileImpl<ElfTypes>(file, start, file_length, file_location, low_4gb, reservation));
...
template <typename ElfTypes>
bool ElfFileImpl<ElfTypes>::Setup(std::string* error_msg) {
...
template <typename ElfTypes>
bool ElfFileImpl<ElfTypes>::Load(bool executable, /*out*/ std::string* error_msg) {
  if (executable) {
  ...
  for (Elf_Word i = 0; i < GetProgramHeaderNum(); i++) {
    ...
    if (!reserved) {
      ...
      MemMap local_reservation = MemMap::MapAnonymous(
          ...
          (reservation_ != nullptr) ? reservation_->Begin() : nullptr,
          vaddr_size,
          PROT_NONE,
          low_4gb_,
          /* reuse= */ false,
  ...
  if (!CheckSectionsExist(file_, error_msg)) {
    return false;
  }
...
ElfFile* ElfFile::Open(File* file,
  ...
  if (header[EI_CLASS] == ELFCLASS64) {
    return ElfFileImpl64::Open(
        file, start, file_length, file_location, low_4gb, reservation, error_msg);
  } else if (header[EI_CLASS] == ELFCLASS32) {
    return ElfFileImpl32::Open(
        file, start, file_length, file_location, low_4gb, reservation, error_msg);
  } else {
