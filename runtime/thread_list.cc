...
std::optional<std::string> ThreadList::WaitForSuspendBarrier(AtomicInteger* barrier,
  ...
  // Extra timeout to compensate for concurrent thread dumps, so that we iare less likely to time
  ...
  static constexpr uint64_t kTracingWaitNSecs = 7'200'000'000'000ull;  // 2 hours
  // Thread is stopped for tracing or debugger.
  ...
  while (i < kSuspendBarrierIters) {
    ...
    // Otherwise keep the old dump_adjustment_ns if SigQuitNanoTime() was cleared.
  }
