...
ALWAYS_INLINE inline static bool VerifyFieldAccess(Thread* self,
  ...
  if (kIsSet && field->IsFinal()) {
    constexpr const char* exoplayer = "java.lang.Class<androidx.media3.exoplayer";
    // To make presubmit heppy: there is an app which overrides a field of this class.
    if (strncmp(field->GetDeclaringClass()->PrettyClass().c_str(),
                exoplayer, strlen(exoplayer)) == 0) {
      return true;
    }
