...
struct VirtualThreadParkingVisitor final : public StackVisitor {
  explicit VirtualThreadParkingVisitor(Thread* thread, std::vector<ShadowFrame*>& shadow_frames)
...
static void Thread_parkVirtualInternal(
  ...
  auto vtf_class =
      hs.NewHandle(soa.Decode<mirror::Class>(WellKnownClasses::dalvik_system_VirtualThreadFrame));
  auto vtps_class = hs.NewHandle(
      soa.Decode<mirror::Class>(WellKnownClasses::dalvik_system_VirtualThreadParkedStates));
  auto vtc_class =
      hs.NewHandle(soa.Decode<mirror::Class>(WellKnownClasses::dalvik_system_VirtualThreadContext));
  auto vtf_array_class =
      hs.NewHandle(cl->FindSystemClass(self, "[Ldalvik/system/VirtualThreadFrame;"));
  auto object_array_class = hs.NewHandle(cl->FindSystemClass(self, "[Ljava/lang/Object;"));
  ...
  Runtime* runtime = Runtime::Current();
  auto frames_h = hs.NewHandle(
      mirror::ObjectArray<mirror::Object>::Alloc(self, vtf_array_class.Get(), num_frames));
  for (size_t i = 0; i < num_frames; i++) {
    const ShadowFrame* sf = shadow_frames[i];
    DCHECK(sf != nullptr);
    ...
    auto vtf =
        hs2.NewHandle(vtf_class->Alloc(self, Runtime::Current()->GetHeap()->GetCurrentAllocator()));
    ...
    auto refs = hs2.NewHandle(
        mirror::ObjectArray<mirror::Object>::Alloc(self, object_array_class.Get(), num_vergs));
    DCHECK(!vtf.IsNull());
    DCHECK(!frame_bytes.IsNull());
    DCHECK(!refs.IsNull());

    bool isRefsAllNull = true;
  ...
  if (self->IsExceptionPending()) {
    // It's likely a OOME. Let's throw it in the java level.
    // Consider handle it in another way, e.g. pinning virtual thread, in the future.
    return;
  }
