...
void fillVirtualThreadFrame(Thread* self, ShadowFrame* frame) {
  ...
  StackHandleScope<8u> hs(self);
  auto jpeer = hs.NewHandle(self->GetPeer());
  auto v_context = hs.NewHandle(WellKnownClasses::java_lang_Thread_target->GetObject(jpeer.Get()));
  DCHECK(v_context->GetClass()->DescriptorEquals("Ldalvik/system/VirtualThreadContext;"))
      << frame->GetMethod()->PrettyMethod();
  auto parked_states =
      hs.NewHandle(WellKnownClasses::dalvik_system_VirtualThreadContext_parkedStates->GetObject(
          v_context.Get()));
  DCHECK(!parked_states.IsNull()) << frame->GetMethod()->PrettyMethod();
  auto frames = hs.NewHandle(WellKnownClasses::dalvik_system_VirtualThreadParkedStates_frames
                                 ->GetObject(parked_states.Get())
  ...
  auto src_frame = hs.NewHandle<mirror::Object>(nullptr);
  for (int32_t i = frames_size - 1; i >= 0; i--) {
    auto obj = frames->Get(i);
    if (!obj.IsNull()) {
      if (frame_index != -1) {
      ...
      frame_index = i;
      src_frame.Assign(obj);
    }
  }

  DCHECK(!src_frame.IsNull()) << frame->GetMethod()->PrettyMethod();
  auto frame_jbytes = hs.NewHandle(
      WellKnownClasses::dalvik_system_VirtualThreadFrame_frame->GetObject(src_frame.Get())
          ->AsByteArray());
  DCHECK(!frame_jbytes.IsNull()) << frame->GetMethod()->PrettyMethod();
  auto refs = hs.NewHandle(
      WellKnownClasses::dalvik_system_VirtualThreadFrame_refs->GetObject(src_frame.Get()));
  ...
  if (!refs.IsNull()) {
    ...
    auto objs = hs.NewHandle(refs->AsObjectArray<mirror::Object>());
