...
// would happen if we created two instances of MarkCompact.
class YoungMarkCompact final : public GarbageCollector {
 public:
  ...
  void RevokeAllThreadLocalBuffers() override { UNREACHABLE(); };
  ...
  void ProcessMarkStack() override { UNREACHABLE(); }
...
class MarkCompact final : public GarbageCollector {
 ...
 private:
  ...
  // mid-gen. During compaction, it represents 'post-compacted' end of mid-en.
  // This is done in PrepreForCompaction(). At the end of GC, in FinishPhase(),
