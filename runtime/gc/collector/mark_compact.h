...
class MarkCompact final : public GarbageCollector {
 ...
 private:
  ...
  void ProcessMarkObject(mirror::Object* obj) REQUIRES_SHARED(Locks::mutator_lock_)
      REQUIRES(Locks::heap_bitmap_lock_);
  // Called to assess if it's safe to use MOVE ioctl. 'additional_mode' may
  // contain additional mode flags the caller may want to pass on to the kernel.
  bool MoveIoctlKernelCheck(uint64_t additional_mode);
  ...
  // Declared volatile to ensure that compiler doesn't cache the value in a
  // register during the loop in GetFreePagesForMapping(). The function can be
  // called by mutators and gc-thread, while only the latter can update it.
