...
bool MarkCompact::SigsysHandler(siginfo_t* info, void* context) {
  ...
  if (info->si_code == SYS_SECCOMP && info->si_syscall == __NR_ioctl) {
    ucontext_t* uctxt = reinterpret_cast<ucontext_t*>(context);
#ifdef __aarch64__
    if (uctxt->uc_mcontext.regs[1] == UFFDIO_MOVE) {
      uctxt->uc_mcontext.regs[0] = -EINVAL;
      return true;
    }
#else
    if (uctxt->uc_mcontext.arm_r1 == UFFDIO_MOVE) {
      uctxt->uc_mcontext.arm_r0 = -EINVAL;
      return true;
    }
#endif
  }
