...
MarkCompact::MarkCompact(Heap* heap)
    : GarbageCollector(heap, "concurrent mark compact"),
      ...
      young_gen_(false),
      use_generational_(heap->use_generational_cc_),
      compacting_(false),
...
// surviving mid-gen objects get promoted to old-gen, while the surviving young
// gen objects are promoted to mid-gen.
//
...
// when objects are promoted to old-gen from mid-gen, their bits are first
// collected in a BitVector and then later copied into mark-bitmap in
...
// references when visiting old-gen objects during compaction. However, we don't
// visit native roots at the time. So native roots are covered during marking
// phase.
  ...
  live_words_bitmap_->VisitLiveStrides(
      ...
      [&](uint32_t stride_begin, size_t stride_size, [[maybe_unused]] bool is_last)
          REQUIRES_SHARED(Locks::mutator_lock_) {
            ...
            const size_t stride_begin_bytes = stride_begin * kAlignment;
