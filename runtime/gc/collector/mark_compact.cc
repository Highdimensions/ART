...
static const bool gUseMoveIoctl = true;
#endif
...
MarkCompact::MarkCompact(Heap* heap)
  ...
  CHECK(madvise(moving_space_begin, moving_space_size, MADV_NOHUGEPAGE) == 0) << strerror(errno);
  // Initialize GC metrics.
  metrics::ArtMetrics* metrics = GetMetrics();
...
void MarkCompact::InitializePhase() {
  ...
  moving_space_end_ = bump_pointer_space_->Limit();
  last_reclaimable_page_ = moving_space_end_;
...
// corresponding post-compact addresses, aligned down to page-size. Therefore,
// some object's beginning portion maybe in old-gen, while the rest is in mid-gen.
// Aligning down is essential as mid_gen_end_ becomes old_gen_end_ at the end of
...
bool MarkCompact::PrepareForCompaction() {
  ...
  if (use_generational_) {
    ...
    // which was done at of previous GC. Compute, 'post-compacted' end of
    ...
    if (first_obj != nullptr) {
      ...
      // the consequences of that at different places. Aligning down is
      // preferred (over aligning up) to avoid pre-mature promotion of young
      // objects to old-gen.
      mid_gen_end_ = AlignDown(reinterpret_cast<uint8_t*>(first_obj), gPageSize);
    } else {
      // Young-gen is empty.
  ...
  if (old_gen_end_ < mid_gen_end_) {
    mid_to_old_promo_bit_vec_.reset(new BitVector((mid_gen_end_ - old_gen_end_) / kObjectAlignment,
                                                  /*expandable=*/false,
  ...
  if (!uffd_initialized_ && CreateUserfaultfd(/*post_fork=*/false)) {
    ...
    if ((gUffdFeatures & UFFD_FEATURE_MOVE) != 0 && gUseMoveIoctl) {
      if (Runtime::Current()->IsZygote()) {
        ...
        // TODO: remove this code once we are sure that app-compat issues are
        // taken care of.
...
bool MarkCompact::FreeFromSpacePages(size_t cur_page_idx, int mode, size_t end_idx_for_mapping) {
  ...
  if (reclaim_begin < last_reclaimable_page_) {
...
uint8_t* MarkCompact::GetFreePageForMapping(size_t size, bool atomic) {
  ...
  if (atomic) {
    while (expected - last_reclaimable_page_ >= static_cast<ssize_t>(size)) {
      uint8_t* desired = expected - size;
      // 'expected' gets updated by compare_exchange_strong() on failure
      if (cur_reclaimable_page_.compare_exchange_strong(
              expected, desired, std::memory_order_relaxed)) {
        return desired;
      }
    }
  } else {
    if (expected - last_reclaimable_page_ >= static_cast<ssize_t>(size)) {
      uint8_t* desired = expected - size;
...
void MarkCompact::CompactMovingSpace(uint8_t* page) {
  ...
  while (idx > black_dense_end_idx) {
    ...
    if (kMode == kFallbackMode) {
      ...
      page = GetFreePageForMapping(gPageSize, use_move_ioctl_);
      if (page == nullptr) {
        page = reserve_page;
      } else {
...
#define PARM2_REG(ctxt) REG(ctxt, REG_R1)
#define RET_REG(ctxt)   REG(ctxt, REG_R0)
...
#define PARM2_REG(ctxt) REG(ctxt, REG_A1)
#define RET_REG(ctxt)   REG(ctxt, REG_A0)
#else
#error "unsupported architecture"
  ...
  if (info->si_code == SYS_SECCOMP && info->si_syscall == __NR_ioctl) {
    ...
    if (PARM2_REG(uctxt) == static_cast<int>(UFFDIO_MOVE)) {
...
void MarkCompact::ConcurrentlyProcessMovingPage(uint8_t* fault_page,
  ...
  if (first_obj == nullptr) {
    ...
    if (use_move_ioctl_) {
      uint8_t* free_page = GetFreePageForMapping(gPageSize, /*atomic=*/true);
