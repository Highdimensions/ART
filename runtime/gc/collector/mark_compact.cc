...
struct uffdio_move {
  ...
  uint64_t src;
  uint64_t len;
  // Modes allowed by MOVE ioctl to change its behavior under certain curcumstances.
  uint64_t mode;
  // Number of bytes moved, or the error encountered. Set by the ioctl.
  int64_t move;
};
#ifndef _UFFDIO_MOVE
...
#endif
// This flag makes the ioctl return successfully even if the src page(s) doesn't exist.
...
bool MarkCompact::MoveIoctlKernelCheck(uint64_t additional_mode) {
    ...
    if ((gUffdFeatures & UFFD_FEATURE_MOVE) != 0 && gMoveIoctlRequested) {
      ...
      static bool safe_to_use_move = [&]() {
        if (kIsTargetAndroid) {
          if (!IsKernelVersionAtLeast(6, 7)) {
            ...
            bool success = MoveIoctlKernelCheck(1ull << 63);
            if (!success) {
          ...
          DCHECK_EQ(ret, 3);
          return minor > 13 || (minor == 13 && lts > 7) || (minor == 12 && lts > 19);
        }
      }();
      ...
      if (safe_to_use_move) {
        if (Runtime::Current()->IsZygote()) {
          ...
          use_move_ioctl_ = MoveIoctlKernelCheck(/*additional_mode=*/0);
          if (!use_move_ioctl_) {
