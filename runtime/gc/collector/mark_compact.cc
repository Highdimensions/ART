...
class MarkCompact::ThreadRootsVisitor : public RootVisitor {
 public:
  ...
  ~ThreadRootsVisitor() {
    if (overflow_arr_ != nullptr) {
      CHECK_GT(start_, overflow_arr_);
      auto pair = std::make_pair(overflow_arr_, start_ - overflow_arr_);
      ...
      // we need to clear the stale references that are unused in the stack.
      for (; start_ < end_; start_++) {
        start_->Assign(nullptr);
      }
    }
  }
 ...
 private:
  void FetchBuffer() REQUIRES_SHARED(Locks::mutator_lock_) {
    DCHECK(overflow_arr_ == nullptr);
    if (mark_compact_->mark_stack_->AtomicBumpBack(kBufferSize, &start_, &end_)) {
      return;
    }
    overflow_arr_ = static_cast<RefType*>(malloc(kBufferSize * sizeof(RefType)));
    start_ = overflow_arr_;
  ...
  void Push(mirror::Object* obj) REQUIRES_SHARED(Locks::mutator_lock_)
    ...
    if (UNLIKELY(start_ == end_)) {
      if (LIKELY(overflow_arr_ == nullptr)) {
        FetchBuffer();
      } else {
        DCHECK_GT(end_, overflow_arr_);
        ptrdiff_t cur_size = end_ - overflow_arr_;
        overflow_arr_ =
            static_cast<RefType*>(realloc(overflow_arr_, 2 * cur_size * sizeof(RefType)));
        start_ = overflow_arr_ + cur_size;
