...
class TraceData {
 public:
  ...
  std::unordered_set<ArtMethod*>& GetTracedMethods() {
    return traced_methods_;
  }

  std::unordered_map<size_t, std::string>& GetTracedThreads() {
...
class TraceProfiler {
 public:
  ...
  // Get the long running methods as a string. This is used to in the sigquit handler to record
  ...
  // entries after this processing (for ex: if we have a really deep call stack) then we record a
  // placeholder method exit event and flush all events.
 ...
 private:
  ...
  // Dumps the tracing data into the specifed trace_file
  static void Dump(std::unique_ptr<File>&& trace_file);

  // Stops tracing. Expectes trace_lock_ to be held when calling this method.
