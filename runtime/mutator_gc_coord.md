...
`kRunnable` in `TransitionFromSuspendedToRunnable` we do so as with an
acquire compare-exchange operation to check that no suspension request remains.
...
acquire orderring to read flags in some places to partially mitigate memory
...
more expensive to do correctly than it is to forego the optimization:
...
while _B_ may change it's state back to runnable, and then read the result of
...
: The canonical fix for this lond of `store; load` reordering problem is to make
