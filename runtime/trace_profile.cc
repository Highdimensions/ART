...
namespace {
void RecordMethodEntryForOnStackMethods(Thread* thread, TraceData* trace_data)
  ...
  // Record a dummy method exit event into the buffer so we record method exits for the methods
  // that are currently on stack.
...

  {
    ...
    for (Thread* thread : Runtime::Current()->GetThreadList()->GetList()) {
      ...
      if (trace_type == LowOverheadTraceType::kLongRunningMethods) {
        // Also record the current methods in the buffer.
        RecordMethodEntryForOnStackMethods(thread, trace_data_);
...
void TraceProfiler::DumpThreadMethodInfo(std::unordered_map<size_t, std::string>& traced_threads,
...
void TraceProfiler::DumpTrace(std::unique_ptr<File>&& trace_file) {
  ...
  DumpThreadMethodInfo(traced_threads, traced_methods, trace_file);
  if (!trace_file->Close()) {
...
void TraceProfiler::DumpLongRunningMethodBuffer(uint32_t thread_id,
  ...
  for (uintptr_t* ptr = method_trace_entries + kAlwaysOnTraceBufSize - 1;
    ...
    uintptr_t entry = *ptr;
    if (entry == 0x1) {
      // This is the special dummy exit we added to record all methods on the stack at the start
      // of the trace. Just ignore this entry.
    } else if (entry & 0x1) {
      // Method exit
...
void TraceProfiler::FlushBufferAndRecordTraceEvent(ArtMethod* method,
  ...
  size_t remaining_entries = (processed_events_ptr - *method_trace_curr_ptr);
  size_t index = kAlwaysOnTraceBufSize;
  if (remaining_entries > kMaxEntriesAfterFlush) {
    // If we don't  have sufficient space just record a dummy exit and flush all
    // the existing methods. We have accurate timestamps to filter out these
    // events in a post-processing step. This would happen only when we have
    ...
    // Encode a dummy exit event. This will be ignored when dumping the methods.
...
void TraceProfiler::DumpLongRunningMethods(std::unique_ptr<File>&& trace_file) {
  ...
  if (old_data.length() > 0) {
    if (!trace_file->WriteFully(old_data.c_str(), old_data.length())) {
      PLOG(WARNING) << "Failed writing information to file";
    }
  }
