...
class ClassLinker::LoadClassHelper {
 public:
  LoadClassHelper(
      Runtime* runtime, const DexFile& dex_file, uint16_t hotness_count, bool is_interface)
      : runtime_(runtime),
        dex_file_(dex_file),
 ...
 private:
  // Loaded data for `ArtField`.
  struct ArtFieldData {
    uint32_t access_flags;
    uint32_t dex_field_index;
  };

  // Loaded for `ArtMethod`.
...
inline void ClassLinker::LoadClassHelper::LinkCode(ArtMethodData* method,
  ...
  if (!ArtMethod::IsInvokable(method->access_flags)) {
    method->entrypoint = runtime_->GetClassLinker()->GetQuickToInterpreterBridgeTrampoline();
...
void ClassLinker::LoadClassHelper::Load(const ClassAccessor& accessor,
  ...
  accessor.VisitFieldsAndMethods([&](
      ...
      const ClassAccessor::Field& field) {
        ...
        LinkCode(method_data, class_def_method_index, &occi);
        DCHECK_EQ(method_data->method_index, 0u);
        ++class_def_method_index;
      });
...
void ClassLinker::LoadClassHelper::FillMethods(ObjPtr<mirror::Class> klass,
  ...
  for (size_t i = 0, size = methods_.size(); i != size; ++i) {
    ...
    dst->SetImtIndexOrHotnessCount(src.imt_index_or_hotness_count);
    ...
    if (!runtime_->IsAotCompiler()) {
      runtime_->GetInstrumentation()->InitializeMethodsCode(dst, src.entrypoint);
    }
  }
}
...
void ClassLinker::LoadClass(Thread* self,
  ...
  Runtime* const runtime = Runtime::Current();
  {
    ...
    uint16_t hotness_count = runtime->GetJITOptions()->GetWarmupThreshold();
    LoadClassHelper helper(runtime, dex_file, hotness_count, klass->IsInterface());
    {
