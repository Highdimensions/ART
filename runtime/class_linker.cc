...
void ClassLinker::LoadClassHelper::FillMethods(ObjPtr<mirror::Class> klass,
  ...
  for (size_t i = 0, size = methods_.size(); i != size; ++i) {
    ...
    // Note: We set the appropriate field of the union (`imt_index_` or `hotness_count_` to satisfy
    // the C++ standard requirements for the active union member but we expect the C++ compiler to
    // optimize away the condition and just copy the `imt_index_or_hotness_count` directly.
    if ((src.access_flags & kAccAbstract) != 0) {
