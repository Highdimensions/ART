...
void* Thread::CreateCallback(void* arg) {
  ...
  }
  {
    ...
    if (UNLIKELY(!runnable.IsNull() && runnable->InstanceOf(v_context_class))) {
      ...
      // stack.
      WellKnownClasses::java_lang_Rnnnable_run->InvokeInterface<'V'>(self, runnable);
...
std::unique_ptr<Context> Thread::QuickDeliverException(bool skip_method_exit_callbacks) {
  ...
  bool is_parking_vthread = this->AreVirtualThreadFlagsEnabled(VirtualThreadFlag::kParking);
