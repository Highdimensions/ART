...
class MethodVerifierImpl : public ::art::verifier::MethodVerifier {
 ...
 protected:
  ...
  NO_INLINE void FailForCopyReference(uint32_t vdst, uint32_t vsrc, const RegType& type)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD)
        << "copy-object v" << vdst << "<-v" << vsrc << " type=" << type;
  }
      ...
      if (0 != work_insn_idx_ && !GetInstructionFlags(work_insn_idx_).IsBranchTarget()) {
        ...
        switch (prev_inst.Opcode()) {
          ...
          case Instruction::CONST_CLASS: {
            ...
            if (prev_inst.VRegA_21c(inst_data) == vreg) {
              work_line_->CopyReference(prev2_inst.VRegA_21c(inst_data), vreg, reg_type);
            } else if (prev2_inst.VRegA_21c(inst_data) == vreg) {
              work_line_->CopyReference(prev_inst.VRegA_21c(inst_data), vreg, reg_type);
            }
            break;
          }
