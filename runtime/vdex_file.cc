...
std::unique_ptr<VdexFile> VdexFile::OpenFromDm(const std::string& filename,
  ...
  std::string vdex_filename = filename + kZipSeparator + kVdexNameInDmFile;
  // The vdex in dm must be uncompressed and page-aligned, to be loaded at the address in the odex.
  FileWithRange vdex_file_with_range = OS::OpenFileDirectlyOrFromZip(
      vdex_filename, kZipSeparator, /*alignment=*/MemMap::GetPageSize(), error_msg);
