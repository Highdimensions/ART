...
class Runtime {
 public:
  ...
  struct LockRecord {
    bool warned = false;
    GcRoot<mirror::Throwable> backtrace;
  };

  struct LockDeps {
    LockDeps() : mutex("LockDeps") {}

    // Map from hashcode to backtrace generated by Thread::CreateInternalStackTrace
    std::unordered_map<int32_t, std::unique_ptr<LockRecord>> deps GUARDED_BY(mutex);
    ReaderWriterMutex mutex;
  };

  static bool IsLockdepEnabled();
  void VisitLockdepRoots(RootVisitor* visitor) REQUIRES_SHARED(Locks::mutator_lock_);
 ...
 private:
  ...
  std::unordered_map<uint32_t, std::unique_ptr<LockDeps>> lock_deps_ GUARDED_BY(lock_deps_mutex_);
  ReaderWriterMutex lock_deps_mutex_;
