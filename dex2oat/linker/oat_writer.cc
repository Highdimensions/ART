...
class OatWriter::LayoutReserveOffsetCodeMethodVisitor : public OrderedMethodVisitor {
 ...
 private:
  ...
  uint32_t NewQuickCodeOffset(CompiledMethod* compiled_method,
    ...
    // `offset_` is relative to the oat data, but we need to align the code relatively to the
    // begining of the oat file to make it aligned in the memory, so we need to use the file
    // offset here.
 ...
 public:
  ...
  bool VisitMethod(const OrderedMethodData& method_data) override
    ...
    if (method_offsets.code_offset_ > offset_) {
      ...
      DCHECK_ALIGNED_PARAM(offset_ + sizeof(OatQuickMethodHeader) + writer_->GetOatDataOffset(),
                           GetInstructionSetCodeAlignment(compiled_method->GetInstructionSet()));
      DCHECK_EQ(
    ...
    #define DO_TRAMPOLINE(field) \
      do { \
        ...
        uint32_t aligned_offset = \
            GetOffsetFromOatDataAlignedToFile(relative_offset + 4, \
            GetInstructionSetCodeAlignment(instruction_set)); \
...
bool OatWriter::WriteDexLayoutSections(OutputStream* oat_rodata,
  ...
  for (size_t i = 0, size = opened_dex_files.size(); i != size; ++i) {
    ...
    const size_t padding_size =
        GetOffsetFromOatDataAlignedToFile(rodata_offset, alignof(DexLayoutSections)) -
        rodata_offset;
    if (padding_size != 0u) {
