...
public class DuplicateStrings {
    ...
    private static Map<Integer, String> mStringIds = null;
    ...
    public static String getStringFromId(int id) {
        if (mStringIds == null || !mStringIds.containsKey(id)) {
            return null;
        }
        return mStringIds.get(id);
    }
    ...
    public static int getIdFromString(String str) {
        if (mStringIds == null) return -1;
        for (Map.Entry<Integer, String> entry : mStringIds.entrySet()) {
            if (str != entry.getValue()) continue;
            return entry.getKey();
        }
        return -1;
    }
    ...
    private static void sortAndAssignIDs(Map<String, DuplicatedString> duplicatedStrings) {
        if (duplicatedStrings == null || duplicatedStrings.isEmpty()) {
            System.out.println("SORT AND ASSIGN NULL");
            return;
        } else {
        ...
        for (int i = 0; i < sortedList.size(); i++) {
            ...
            mStringIds.put(i + 1, sortedList.get(i));
        }
    }
