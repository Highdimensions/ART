...
TEST_F(ArtdTest, maybeCreateSdc) {
  ...
  std::string sdm_file = OR_FAIL(BuildSecureDexMetadataPath(sdm_sdc_paths_));
  std::string sdc_file = OR_FAIL(BuildSecureDexMetadataCompanionPath(sdm_sdc_paths_));
  CreateFile(sdm_file);

  ASSERT_STATUS_OK(artd_->maybeCreateSdc(
      {.sdcPath = sdm_sdc_paths_, .permissionSettings = permission_settings_}));

  CheckContent(sdc_file, StartsWith("sdm-timestamp="));
}
...
TEST_F(ArtdTest, maybeCreateSdcAlreadyCreated) {
  ...
  struct stat sdm_st;
  ASSERT_EQ(stat(sdm_file.c_str(), &sdm_st), 0);

  std::filesystem::create_directories(std::filesystem::path(sdc_file).parent_path());
  std::unique_ptr<File> file(OS::CreateEmptyFileWriteOnly(sdc_file.c_str()));
  ASSERT_NE(file, nullptr) << strerror(errno);
  SdcWriter writer(std::move(*file));
  writer.SetSdmTimestamp(sdm_st.st_mtim.tv_sec);
  writer.SetApexVersions("123456789");
  ...
  EXPECT_EQ(sdc_st.st_mtim.tv_sec, new_sdc_st.st_mtim.tv_sec);
  EXPECT_EQ(sdc_st.st_mtim.tv_nsec, new_sdc_st.st_mtim.tv_nsec);
}
