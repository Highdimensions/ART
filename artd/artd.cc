...
ndk::ScopedAStatus Artd::maybeCreateSdc(const OutputSecureDexMetadataCompanion& in_outputSdc) {
  ...
  if (in_outputSdc.permissionSettings.seContext.has_value()) {
    return Fatal("'seContext' must be null because SDM files are for primary dex files");
  }

  std::string sdm_path = OR_RETURN_FATAL(BuildSecureDexMetadataPath(in_outputSdc.sdcPath));
  std::string sdc_path = OR_RETURN_FATAL(BuildSecureDexMetadataCompanionPath(in_outputSdc.sdcPath));
  ...
  if (sdc_reader != nullptr && sdc_reader->GetSdmTimestamp() == sdm_st.st_mtim.tv_sec) {
  ...
  if (!in_outputSdc.sdcPath.isInDalvikCache) {
    ...
    // Unlike `dexopt`, we only need one restorecon here because SDM files are for primary dex
    // files, whose oat directory doesn't have an MLS label.
  ...
  writer.SetSdmTimestamp(sdm_st.st_mtim.tv_sec);
...
ScopedAStatus Artd::cleanup(
  ...
  for (const SecureDexMetadataWithCompanionPaths& sdm_sdc : in_SdmSdcFilesToKeep) {
    files_to_keep.insert(OR_RETURN_FATAL(BuildSecureDexMetadataPath(sdm_sdc)));
    files_to_keep.insert(OR_RETURN_FATAL(BuildSecureDexMetadataCompanionPath(sdm_sdc)));
  }
...
ScopedAStatus Artd::deleteSdmSdcFiles(const SecureDexMetadataWithCompanionPaths& in_SdmSdcPaths,
  ...
  std::string sdm_path = OR_RETURN_FATAL(BuildSecureDexMetadataPath(in_SdmSdcPaths));
  std::string sdc_path = OR_RETURN_FATAL(BuildSecureDexMetadataCompanionPath(in_SdmSdcPaths));
...
ndk::ScopedAStatus Artd::getSdmFileSize(const SecureDexMetadataWithCompanionPaths& in_sdmPath,
                                        int64_t* _aidl_return) {
  RETURN_FATAL_IF_PRE_REBOOT(options_);
  std::string sdm_path = OR_RETURN_FATAL(BuildSecureDexMetadataPath(in_sdmPath));
