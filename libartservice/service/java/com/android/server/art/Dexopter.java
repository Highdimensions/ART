...
public abstract class Dexopter<DexInfoType extends DetailedDexInfo> {
            ...
            try {
                ...
                for (Abi abi : getAllAbis(dexInfo)) {
                    ...
                    try {
                        ...
                        if (!mInjector.isPreReboot()) {
                            if (verifySdmSignature(target.dexInfo().dexPath(),
                                        getSdmPath(target.dexInfo().dexPath(), target.isa()))) {
                                createSdc(target, permissionSettings);
                            }
                        }
    ...
    private boolean verifySdm(@NonNull DexoptTarget<DexInfoType> target) throws RemoteException {
        ...
        String sdmPath = getSdmPath(dexPath, target.isa());
    ...
    private void createSdc(@NonNull DexoptTarget<DexInfoType> target,
        ...
        SecureDexMetadataPath sdmPath =
                AidlUtils.buildSecureDexMetadataPath(target.dexInfo().dexPath(), target.isa());
