...
public abstract class Dexopter<DexInfoType extends DetailedDexInfo> {
            ...
            try {
                ...
                for (Abi abi : getAllAbis(dexInfo)) {
                    ...
                    try {
                        ...
                        if (!mInjector.isPreReboot()) {
                            maybeVerifyAndStampSdm(target, permissionSettings);
                        }
    ...
    private void maybeVerifyAndStampSdm(@NonNull DexoptTarget<DexInfoType> target,
        ...
        String sdmPathStr = getSdmPath(dexPath, target.isa());

        if (!mInjector.fileExists(sdmPathStr)) {
            return;
        }

        try {
            if (mInjector.getArtd().checkSdc(outputArtifacts.artifactsPath, sdmPath)) {
                return;
            }
        } catch (ServiceSpecificException e) {
            AsLog.e("Failed to check sdc for " + sdmPathStr, e);
            return;
        }

        if (!verifySdmSignature(dexPath, sdmPathStr)) {
            return;
        }

        try {
            mInjector.getArtd().createSdc(outputArtifacts, sdmPath);
        } catch (ServiceSpecificException e) {
            AsLog.e("Failed to create sdc for " + sdmPathStr, e);
        }
    }
