...
public class PreRebootDexoptJob implements ArtServiceJobInterface {
    ...
    // It's also guaranteed that the monitor of `this` is notified when `mCancellationSignal` is
    // fired **before `PreRebootDriver.run` is called**, if it happens.
        ...
        mRunningJob = new CompletableFuture().runAsync(() -> {
            ...
            try {
                ...
                if (otaSlot != null && !isUpdateEngineReady && !mapSnapshotsForOta) {
                    ...
                    synchronized (this) {
                        Utils.check(mIsUpdateEngineReady || mCancellationSignal.isCanceled());
                    }
                }
    ...
    private void triggerUpdateEnginePostinstallAndWait() throws UpdateEngineException {
        ...
        // See `ArtShellCommandTest` for typical flows.
