...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("IfLtFp7AElseB_86LoadFromConstantTable(2.0, true): ");
        new Main().$noinline$IfLtFp7AElseB_86LoadFromConstantTable(2.0, true);
        System.out.print("IfLtFp7AElseB_86LoadFromConstantTable(10.0, true): ");
        new Main().$noinline$IfLtFp7AElseB_86LoadFromConstantTable(10.0, true);
        System.out.print("IfLtFp7AElseB_86LoadFromConstantTable(2.0, false): ");
        new Main().$noinline$IfLtFp7AElseB_86LoadFromConstantTable(2.0, false);
        System.out.print("IfLtFp7AElseB_86LoadFromConstantTable(10.0, false): ");
        new Main().$noinline$IfLtFp7AElseB_86LoadFromConstantTable(10.0, false);
    ...
    private static boolean $inline$XLtFp7(double x) {
        return x < 7.0;
    }
    ...
    /// CHECK-START: void Main.$noinline$IfLtFp7AElseB_86LoadFromConstantTable(double, boolean) prepare_for_register_allocation (before)
    /// CHECK:      <<Cond:z\d+>> {{LessThan|GreaterThanOrEqual}} emitted_at_use_site:false
    /// CHECK:                    If [<<Cond>>]

    /// CHECK-START: void Main.$noinline$IfLtFp7AElseB_86LoadFromConstantTable(double, boolean) prepare_for_register_allocation (after)
    /// CHECK:      <<Cond:z\d+>> {{LessThan|GreaterThanOrEqual}} emitted_at_use_site:true
    /// CHECK:                    If [<<Cond>>]

    /// CHECK-START-X86: void Main.$noinline$IfLtFp7AElseB_86LoadFromConstantTable(double, boolean) prepare_for_register_allocation (after)
    ...
    public static void $noinline$IfLtFp7AElseB_86LoadFromConstantTable(
            double x, boolean otherCond) {
        boolean cond = $inline$XLtFp7(x);
        if (otherCond) {
            if (cond) {
                cond = false;  // Avoid environment use below.
                $noinline$A();
