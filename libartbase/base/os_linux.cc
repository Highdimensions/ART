...
FileWithRange OS::OpenFileDirectlyOrFromZip(const std::string& name_and_zip_entry,
  ...
  off_t start = 0;
  std::optional<size_t> total_file_length = file->GetLength(error_msg);
  if (!total_file_length.has_value()) {
    return FileWithRange::Invalid();
  }
  size_t length = total_file_length.value();
  ...
  if (!zip_entry_name.empty()) {
    ...
    if (start + length > total_file_length) {
