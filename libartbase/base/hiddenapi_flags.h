...
class ApiList {
 private:
  ...
  // can be Combine()'ed with another ApiList with a Value to produce a valid combination.
 ...
 public:
  ...
  static bool FromNames(std::vector<std::string>::iterator begin,
    ...
    // Note that Combine() will use the other value if joined with an invalid one.
  ...
  static ApiList Combine(const ApiList& api1, const ApiList& api2) {
    ...
    if (val1 == val2) {
      ...
      // We include the domain from val1, so that it works to join a
      // FromDomainApi() instance with another one that has a Value.
