...
class HiddenApiTest;  // For later friend declaration.
...
class ApiList {
 private:
  ...
  // that.
  //
  // Currently we have the invariant that only the app domain (with no DomainApi
  // bits set) uses Value. In other API domains it is always kInvalid.
  uint32_t dex_flags_;
  ...
  static ApiList Join(const ApiList& api1, const ApiList& api2) {
    ...
    if (val1 == val2) {
      ...
      // TODO: This ought not use the DomainApi from the invalid ApiList.
  ...
  friend class ::art::HiddenApiTest;
 ...
 public:
  ...
  static ApiList FromName(const std::string& str) {
    for (uint32_t i = 0; i < kValueCount; ++i) {
      if (str == kValueNames[i]) {
        return FromValue(helper::GetEnumAt<Value>(i));
      }
    }
  ...
  static bool FromNames(std::vector<std::string>::iterator begin,
    ...
    // Note that Join() will use the other value if joined with an invalid one.
    ...
    for (std::vector<std::string>::iterator it = begin; it != end; it++) {
      ...
      api_list = Join(api_list, current);
    }
