...
void FastCompilerARM64::MoveConstantsToRegisters() {
  for (uint32_t i = 0; i < vreg_locations_.size(); ++i) {
    ...
    if (location.IsConstant()) {
      vreg_locations_[i] = CreateNewRegisterLocation(i, DataType::Type::kInt32, /* next= */ nullptr);
...
bool FastCompilerARM64::If_21_22t(const Instruction& instruction, uint32_t dex_pc) {
  ...
  // !kCompareWithZero
  // Reload the location now that we have moved constants to registers.
  ...
  if (location.IsConstant() && other_location.IsConstant()) {
    ...
    return true;
  } else if (location.IsRegister() && other_location.IsRegister()) {
    CPURegister reg = CPURegisterFrom(location, DataType::Type::kInt32);
...
#define IF_XX(comparison, cond) \
    ...
    case Instruction::RETURN_OBJECT: {
      ...
      if (has_frame_) {
        ...
        // register" optimization, so move the returned register back to a
        // callee save location in case the method has a frame.
