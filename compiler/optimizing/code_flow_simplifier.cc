...
// Search `block` for phis that has different inputs at `index1` and `index2`.
...
bool HCodeFlowSimplifier::TryFlattenGotoBlock(HBasicBlock* block,
...
bool HCodeFlowSimplifier::Run() {
  ...
  // Mark visited blocks by block id for reverse post order fixup in `TryFlattenGotoBlock()`.
  ...
  while (reverse_post_order_index != /* Do not process entry block with index 0. */ 1u) {
    ...
    if (last->IsGoto()) {
      if (last == block->GetFirstInstruction() &&
          TryFlattenGotoBlock(block, reverse_post_order_index, &visited_blocks)) {
        did_simplify = true;
