...
void IntrinsicCodeGeneratorRISCV64::VisitMethodHandleInvokeExact(HInvoke* invoke) {
  ...
  __ Addi(temp, temp, -mirror::MethodHandle::Kind::kInvokeStatic);
  __ Bne(temp, A0, slow_path->GetEntryLabel());

  __ Loadwu(temp, method_handle, mirror::MethodHandle::ArtFieldOrMethodOffset().Int32Value());
  Offset entry_point = ArtMethod::EntryPointFromQuickCompiledCodeOffset(kRiscv64PointerSize);
  __ Loadd(RA, temp, entry_point.SizeValue());
  __ Jalr(RA);
