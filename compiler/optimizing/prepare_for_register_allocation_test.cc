...
TEST_F(PrepareForRegisterAllocationTest, MergeCondition_RefInEnv) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  HBasicBlock* ret = InitEntryMainExitGraphWithReturnVoid(&vshs);
  auto [start, left, right_end] = CreateDiamondPattern(ret);
  ...
  // after the invoke would not extend the lifetime of `param` for the purpose of GC.
...
TEST_F(PrepareForRegisterAllocationTest, MergeConditionPrevented_RefNotInEnv) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  HBasicBlock* ret = InitEntryMainExitGraphWithReturnVoid(&vshs);
  auto [start, left, right_end] = CreateDiamondPattern(ret);
  ...
  // This invoke's environment does not contain `param`, so reordering the `condition`
  // after the invoke would need to extend the lifetime of `param` for the purpose of GC.
  // We do not want to extend lifetime of references, therefore the optimization is skipped.
  HInvoke* invoke = MakeInvokeStatic(start, DataType::Type::kVoid, /*args=*/ {}, /*env=*/ {});
