...
class GlobalValueNumberer : public ValueObject {
 public:
  GlobalValueNumberer(HGraph* graph, const SideEffectsAnalysis& side_effects)
      : graph_(graph),
        ...
        dominated_to_visit_(allocator_.Adapter(kArenaAllocGvn)),
        successors_to_visit_(allocator_.Adapter(kArenaAllocGvn)),
    ...
    dominated_to_visit_.reserve(graph->GetBlocks().size());
    successors_to_visit_.reserve(graph->GetBlocks().size());
    for (HBasicBlock* block : graph->GetActiveBlocks()) {
 ...
 private:
  ...
  // Number of dominated blocks left to visit in GVN.
  ScopedArenaVector<uint32_t> dominated_to_visit_;
  // Number of successor blocks left to visit in GVN.
  ScopedArenaVector<uint32_t> successors_to_visit_;
  // True iff the set is free to be reused by another block.
