...
bool HInstructionBuilder::BuildInvoke(const Instruction& instruction,
  ...
  // Now, when invokeExact is implemented as an intrinsic, compiler's attempt to devirtualize such
  // ill-formed virtual calls can lead to compiler crashes as the intrinsic expects arguments to be
  // set up in a different manner than it's done for virtual calls.
  // Creating HInvokeUnresolved to make sure that such invoke-virtual calls are not devirtualized
  // and treated as native method calls.
  if (kIsDebugBuild) {
    ScopedObjectAccess soa(Thread::Current());
