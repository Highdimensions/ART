  ...
  static bool CanCall(llvm::Function& function) {
    for (llvm::BasicBlock& block : function) {
      for (llvm::Instruction& inst : block) {
        if ([[maybe_unused]]llvm::IntrinsicInst* intrinsic = llvm::dyn_cast<llvm::IntrinsicInst>(&inst)) {
...
class EliminateUnwindRelocationPhisPass
 ...
 public:
  llvm::PreservedAnalyses run(
    ...
    bool changed = false;
    for (llvm::BasicBlock& basic_block : function) {
      llvm::Instruction* first_inst = basic_block.getFirstNonPHI();
...
class RewritePatchpointsPass : public llvm::PassInfoMixin<RewritePatchpointsPass> {
 public:
  ...
  static llvm::SmallVector<llvm::Value*> GetPatchpointArgs(llvm::GCStatepointInst* statepoint,
    ...
    // GC live values.
    patchpoint_args.append(statepoint->gc_args_begin(), statepoint->gc_args_end());
  ...
  static llvm::CallBase* CreatePatchpointCall(PatchpointKind kind,
    ...
    if (result_type->isVoidTy()) {
      patchpoint_callee =
          llvm::Intrinsic::getDeclaration(module, llvm::Intrinsic::experimental_patchpoint_void);
    } else {
      patchpoint_callee = llvm::Intrinsic::getDeclaration(
 ...
 public:
  ...
  llvm::PreservedAnalyses run(
    ...
    for (auto& [statepoint, statepoint_info] : statepoint_infos) {
      ...
      llvm::CallBase* new_statepoint =
          llvm::CallBase::Create(statepoint, std::move(new_bundle), /* InsertPt= */ statepoint);
    ...
    for (llvm::BasicBlock& basic_block : function) {
      for (llvm::Instruction& inst : basic_block) {
        if (llvm::GCStatepointInst* statepoint = llvm::dyn_cast<llvm::GCStatepointInst>(&inst)) {
          ...
          for (llvm::Value* gc_ptr : statepoint->gc_args()) {
