...
class HCodeFlowSimplifier : public HOptimization {
 ...
 private:
  ...
  // the merge blocks for multi-if/switch code flow. This can reduce the number of parrallel
  ...
  // for `HSelect` simplification and `TryFlattenGotoBlock()` can help. It turns
  //
  //      1 (outer if)
  //     / \
  //    2   3 (inner if)
  ...
  bool TryFlattenGotoBlock(HBasicBlock* block,
