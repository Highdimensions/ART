...
//  CHECK(it_method != dex_files.end() || !codegen_->GetCompilerOptions().WithinOatFile(dex));
  ...
  bool result = DoTryInline(invoke_instruction, forbidden);
...
bool HInliner::DoTryInline(HInvoke* invoke_instruction, bool forbidden) {
  ...
  if (actual_method != nullptr) {
    ...
    bool result = forbidden
        ? false
        : TryInlineAndReplace(invoke_instruction,
                              actual_method,
                              receiver_info,
                              /* do_rtp= */ true,
