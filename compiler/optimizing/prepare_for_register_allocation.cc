...
static bool ConditionMoveWouldExtendReferenceLifetime(HCondition* condition, HInstruction* user) {
  ...
  HInstruction* rhs = condition->InputAt(0);
  ...
  if (instruction_with_env == nullptr) {
    ...
    // moving the `conition` to the `user` would indeed extend the lifetime of a reference.
    return true;
  }
...
bool PrepareForRegisterAllocationVisitor::CanEmitConditionAt(HCondition* condition,
  ...
  if (condition->GetNext() != user && ConditionMoveWouldExtendReferenceLifetime(condition,user)) {
    return false;
  }
