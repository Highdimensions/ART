...
static void MarkReachableBlocks(HGraph* graph, RawBitVector<size_t> visited) {
...
bool HDeadCodeElimination::RemoveEmptyIfs() {
  ...
  for (HBasicBlock* block : graph_->GetPostOrder()) {
    ...
    ScopedArenaAllocator allocator(graph_->GetArenaStack());
    RawBitVector<size_t> visited_blocks =
    ...
    RawBitVector<size_t> visited = ArenaBitVector::CreateFixedSize(
...
bool HDeadCodeElimination::RemoveDeadBlocks(bool force_recomputation,
  ...
  RawBitVector<size_t> live_blocks =
      ArenaBitVector::CreateFixedSize(&allocator, graph_->GetBlocks().size(), kArenaAllocDCE);
