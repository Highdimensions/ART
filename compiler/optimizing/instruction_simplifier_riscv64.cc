...
class InstructionSimplifierRiscv64Visitor final : public HGraphVisitor {
 ...
 private:
  ...
  bool TryOptimizeBinaryOpWithConstWithBitManipulation(HBinaryOperation* op) {
    ...
    if (op->IsOr()) {
      replacement = new (allocator) HRiscv64BitSet(result_type, op_value, shamt);
    } else if (op->IsXor()) {
      replacement = new (allocator) HRiscv64BitInvert(result_type, op_value, shamt);
    } else {
      DCHECK(op->IsAnd());
      replacement = new (allocator) HRiscv64BitClear(result_type, op_value, shamt);
    }
