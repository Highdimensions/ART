...
class ELFFileParser {
 public:
  ...
  // Get the contents of the '.text[.*]' section. This should be used to get the generated machine
  // code instead of GetSectionContents(".text") because LLVM may decide to place functions in a
  // different section, like '.text.unlikely.'.
  llvm::ArrayRef<uint8_t> GetTextSectionContents() const {
...
void DumpStackMap(const llvm::StackMapParser<ELFFileParser::ELFT::Endianness>& stack_map_parser);
