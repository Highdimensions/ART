...
void HConstantFoldingVisitor::VisitStaticFieldGet(HStaticFieldGet* instruction) {
  if (IsSDK_INT(instruction)) {
    // TODO(solanes): Do Runtime::Current()->GetTargetSdkVersion() in ParsedOptions::DoParse once
    // and cache the value?
    if (Runtime::Current()->GetBuildSdkVersion() != static_cast<uint32_t>(SdkVersion::kUnset)) {
      instruction->ReplaceWith(
          GetGraph()->GetIntConstant(Runtime::Current()->GetBuildSdkVersion()));
