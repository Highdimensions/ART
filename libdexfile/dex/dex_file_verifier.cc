...
bool DexFileVerifier::CheckInterClassDataItem() {
  ...
  for (const ClassAccessor::Field& read_field : accessor.GetFields()) {
    ...
    if (seen_fields.find({field.name_idx_, field.type_idx_}) != seen_fields.end()) {
      ErrorStringPrintf("Duplicated field %s with type %s in class %s",
                        dex_file_->GetStringData(field.name_idx_),
                        dex_file_->GetTypeDescriptor(field.type_idx_),
